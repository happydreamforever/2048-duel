...
class CubeGameEngine(
    ...
    override fun applyQuarterTurn(slice: ActiveSlice, direction: Int) {
        if (slice == ActiveSlice.NONE) return
        rotation = RotationState(activeSlice = slice, direction = direction, isAnimating = false)
        rotatedAngle = if (direction >= 0) 90f else -90f
        commitSliceRotation()
        rotatedAngle = 0f
